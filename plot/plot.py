...
class DataGen(object):
    def __init__(self, init=0):
        ...
        self.data = [ float(ord(_)) for _ in self.cxn.read(3) ];
...
class GraphFrame(wx.Frame):
    ...
    def __init__(self):
        for _ in xrange(3):
            for __ in xrange(3):
                self.colors[_][__] = float(self.colors[_][__])/255.0;
    ...
    def init_plot(self):
        ...
        self.axes.set_title('ADC Input Data', size=12)
    ...
    def draw_plot(self):
        ...
        if self.ymin_control.is_auto():
            ymin = round(min([min(_) for _ in self.data]), 0) - 1
        else:
            ymin = int(self.ymin_control.manual_value())
        
        if self.ymax_control.is_auto():
            ymax = round(max([max(_) for _ in self.data]), 0) + 1
        else:
